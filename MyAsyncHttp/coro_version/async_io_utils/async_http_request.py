from MyAsyncHttp.coro_version.loop import get_event_loop
import socket
from MyAsyncHttp.coro_version.fd_manger import FdManger
from MyAsyncHttp.coro_version.http_utils import parse_http_response_header
import select
from MyAsyncHttp.coro_version.furture import Future
...
#             fd_manager.del_fd(fd)
#
# def _epollout_event_callback(future: Future):
#
...
def request(method: str, url: str, header=None):
    ...
    try:
        ...
        client_socket.send(fd_manager[fd].write_buffer)
        ...
        while True:
            ...
            chunked = yield future

            loop.unregister(fd)
            fd_manager[fd].read_buffer += chunked
...
from MyAsyncHttp.coro_version.task import Task
Task(request("GET", "http://www.baidu.com", header=[]))
loop.start()
